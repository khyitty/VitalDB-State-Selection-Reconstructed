...
| 8E - Sealed-test inputs and final evaluation readiness | readiness complete; actual evaluation not started | Implementation `8c752044320c3bd7360d23a0a98927c159f36e13` independently verified on remote `main`; all 490 sealed-test templates and runtime bundles are private and checksum-rooted; BIS/SQI/RFTN20 logical accesses are 490 each with train and propofol access 0; train-only S0/S1 scalers were applied without fitting; Shard A final models are checksum-valid and Shard B remains pending external completion; synthetic-only latent-BIS metrics, subject-level paired statistics, and Holm adjustment passed; 405 base tests completed with 383 passed and 22 established optional-RL skips, all 22 skipped tests passed in the isolated RL environment, and the 29/3/4/3/3/7-source first-N guards passed; actual model episodes 0, final evaluation not started, condition comparison not performed, and best model not selected |
| 8F - Manuscript readiness and aggregate-results publication pipeline | implementation prepared; publication pending | Starting source `2346b964c6c8a0bc1f66953b80c65aae8d4ca800`; English result-pending manuscript, prespecified discussion scenarios, figure/table plan, strict aggregate schema, deterministic renderer, and exact-interface finalization runbook prepared; 16 Phase 8F tests and 12 directly related Phase 8E tests passed (five optional-RL tests skipped in base and passed in the isolated runtime); full suite not run; synthetic fixtures only; actual model loads 0, test episodes 0, condition comparisons false, Shard B accesses 0, and public case/event rows 0; Phase 8E runner semantics unchanged and a separately reviewed aggregation step remains required before real-result rendering |
